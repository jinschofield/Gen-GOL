#!/usr/bin/env python3
"""
Extract and visualize the most common still-life subpatterns from
...
def main():
    p = argparse.ArgumentParser(description="Common still-life subpatterns")
    ...
    p.add_argument('--max_window',  type=int, default=9,
    ...
    p.add_argument('--out_dir',     type=str, default='./common_still_life',
    ...
    with torch.no_grad():
        ...
        print("Sampling conditioned still-life outputs...")
        samples = diffusion.sample(model, shape, c=c)

    # binarize
    grids = (samples.squeeze(1).cpu().numpy() > args.threshold).astype(np.uint8)

    # filter true still-life by simulation
    still = []
    for g in grids:
        hist = simulate(g, steps=args.timesteps)
        per = detect_period(hist)
        if per == 1 and hist[-1].sum() > 0:
            still.append(g)
    print(f"Filtered {len(still)} still-life samples out of {len(grids)}")
    if not still:
        print("No still-life found, exiting.")
        return
    still = np.stack(still)
